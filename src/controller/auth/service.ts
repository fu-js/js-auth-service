...
export const handleGoogleCallback = async (c: Context<{}, any, {}>) => {
    ...
    const isMember = await isEmailAllowed(user.email, sp)

    if (!isMember) {
        return c.redirect(`${redirect}?error=not-allowed`)
    }
...
const isEmailAllowed = async (email: string, supabase: SupabaseClient) => {
